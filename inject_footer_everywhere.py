...
new_footer = """
    <footer class="global-footer" style="padding: 2rem; background-color: #fdfbf7; border-top: 1px solid #eaeaea; text-align: center; font-family: 'Montserrat', sans-serif; z-index: 1000; position: relative;">
...
for filepath in html_files:
    ...
    # Strip out any existing footer completely to avoid duplicates
    content = re.sub(r'<footer class="global-footer">.*?</footer>', '', content, flags=re.DOTALL)
    content = re.sub(r'<footer class="global-footer".*?</footer>', '', content, flags=re.DOTALL)
    
    # Inject before </body>
    if '</body>' in content:
        content = content.replace('</body>', new_footer + '</body>')
    else:
        content += new_footer
...
print(f"Force-injected footer into {len(html_files)} HTML files.")
